...
<br>
A database consists of many tables. These tables can be related through key variables. For instance one table can contain sensor data while another table contains the meta data per device. These tables can then be combined via a shared variable (*field*) containing the device IDs. If the meta data includes location data (e.g. longitude/latitude) you could draw maps with the measured values (click on the markers in the map).
<br>
